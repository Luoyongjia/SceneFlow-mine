"""
including EPE3D, ACCS, ACCR, Outliers, EPE2D, ACC2D
...
def evaluate_3D(sf_pred, sf_gt):
    ...
    sf_gt: (N, 3)
    """
    l2_norm = np.linalg.norm(sf_pred - sf_gt, axit=-1)
    EPE3D = l2_norm.mean()

    gt_norm = np.linalg.norm(sf_gt, axis=-1)
    relative_err = l2_norm / (gt_norm + 1e-4)
